...
function checkVisibility() {
    ...
    const pageHeightThreshold = 800; // 頁面的 800px 高度觸發點
    ...
    // 桌機版 share-fixed sticky 邏輯
    if (!isMobile) {
        const bottomThreshold = 100; // 接近主內容底部 100px 時隱藏

        // 當頁面的 800px 高度進入視窗範圍時顯示
        if (scrollY + windowHeight > pageHeightThreshold && distanceToMainBottom > bottomThreshold) {
            shareFixed?.classList.add('show');
            shareFixed?.setAttribute('aria-hidden', 'false');
            shareFixed.style.position = 'fixed';
            shareFixed.style.top = 'auto';
            shareFixed.style.bottom = '20px'; // 固定在右下角
            shareFixed.style.right = '40px';
            shareFixed.style.transform = 'none'; // 移除 transform 以保持 sticky 效果
        } else {
...
$(document).ready(function () {
  $('.news_list').on('click', function (e) {
    ...
    // 收起其他展開項目
    $('.news_list.active-news').removeClass('active-news')
    ...
    function scrollAdv(){
        ...
        const handleScroll = () => {
            ...
            {           
                const leadeerPicThreshold = leadeerPic?.getBoundingClientRect();
                ...
                if(window.scrollY > prevWinSY )
                {
                    if (window.scrollY > rangeDis - spaceDisAdj)
                    {
                        var leftPosition = rangeDis - spaceDis;
                        leadeerPic.style.top = leftPosition + spaceDisAdj + "px";
                        leadeerPic.style.position = 'absolute';
                    }
                }
                else if(window.scrollY < prevWinSY)
                {
                    if (window.scrollY <= rangeDis - spaceDisAdj)
                    {       
                        leadeerPic.style.top = orgTop + "px";  
                        leadeerPic.style.position = 'fixed';  
                    }
                }
                prevWinSY = window.scrollY;
                updating = false
            }
        }
