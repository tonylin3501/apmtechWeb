document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll(selectors).forEach(el => {
    ...
    const shareIcon = document.getElementById('shareIcon'); // 確保這裡有取得 shareIcon 元素

    const shareIconPath = 'img/share.svg';   // 平時的分享 SVG 路徑
    const closeIconPath = 'img/cancel.svg';  // 關閉時的取消 SVG 路徑

    // 所有的邏輯都放在這一個 if 判斷式裡面
