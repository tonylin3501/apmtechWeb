...
function checkVisibility() {
    ...
    const shareFixed = document.querySelector('.share-fixed');  // 新增：定義 shareFixed
    const backToTop = document.getElementById('backToTop');
    ...
    console.log('checkVisibility: ', { scrollY, windowHeight, mainBottom, distanceToMainBottom, isMobile });

    if (isMobile) {
        if (distanceToMainBottom > paddingFromBottom) {
            // 顯示並設為 fixed 位置，當距離 main 底部大於 60px
            ...
            // 當接近 main 底部 60px 內時，設為 absolute 並固定在底部上方 60px
            shareMobile.classList.add('show');
    ...
    // .share-fixed 的 show 邏輯（參考 0718.js，整合進函數內）
