document.addEventListener('DOMContentLoaded', () => {
    ...
    const menuOverlayLeader = document.querySelector('.menu-overlay-leader');
    ...
    const fontSizeArea = fontsizeBtn?.closest('.nav-icons-fontsize');
    const selectors = 'h1, h2, h3, h4, h5, h6, .display, .body-1, .body-2, .minimum, .largebutton, .middlebutton, h1 .en, h2 .en, h3 .en, h4 .en, h5 .en, h6 .en, .body-1 .en, .body-2 .en';
    const sizeButtons = document.querySelectorAll('.font-size-selector-container .size-button');
    ...
    const sizeOffsets = {
        'size-small': -2,  // 小 = 基準 - 2
        'size-medium': 0,  // 中 = 基準 + 0
        'size-large': 2    // 大 = 基準 + 2
    };

    const DEFAULT_SIZE_KEY = 'size-medium';
    const STORAGE_KEY = 'fontSizePreference';

    // 【新增】媒體查詢檢查物件：大於 991px 適用
    const desktopMediaQuery = window.matchMedia('(min-width: 992px)');
    // -------------------------

    /**
     * 核心函數：根據選擇的尺寸來更新所有文字元素的字體大小
     * 【新增檢查】此函數只在桌面模式下執行
     * @param {string} sizeKey - 選擇的大小 ('size-small', 'size-medium', 'size-large')
     */
    function updateFontSizes(sizeKey) {
        // 如果不是電腦版 (小於 992px)，則終止執行，依賴 CSS
        if (!desktopMediaQuery.matches) {
            return;
        }
        
        const offset = sizeOffsets[sizeKey];
        ...
        allTextElements.forEach(element => {
            ...
            // 優先處理標籤選擇器 (h1, h2, etc.)
            if (baseSizes.hasOwnProperty(tag)) {
                baseSize = baseSizes[tag];
            } 
            // 處理類別選擇器 (.body-1, .display, etc.)
            else {
                for (const selector in baseSizes) {
                    if (selector.startsWith('.') && element.classList.contains(selector.substring(1))) {
                        baseSize = baseSizes[selector];
                        break;
                    }
                }
            }
        ...
        // 更新導航列字體按鈕本身的大小
        if (fontsizeBtn && fontsizeBtn.classList.contains('body-1')) {
            const base = baseSizes['.body-1'];
            fontsizeBtn.style.fontSize = `${base + offset}px`;
        }
    }


    // --- 1. 頁面載入時：檢查是否為電腦版並套用儲存的字體大小 ---
    
    if (desktopMediaQuery.matches) {
        // 從 localStorage 讀取上次儲存的偏好，如果沒有，就使用預設值
        const storedSize = localStorage.getItem(STORAGE_KEY);
        const initialSize = (storedSize && sizeOffsets.hasOwnProperty(storedSize)) ? storedSize : DEFAULT_SIZE_KEY;
        
        // 套用讀取到的字體大小 (僅在桌面版執行)
        updateFontSizes(initialSize);
    }
    
    // 【新增】監聽螢幕寬度變化：如果從手機切換到桌面，自動套用儲存的大小
    desktopMediaQuery.addListener(() => {
        if (desktopMediaQuery.matches) {
            const storedSize = localStorage.getItem(STORAGE_KEY) || DEFAULT_SIZE_KEY;
            updateFontSizes(storedSize);
        } else {
            // 如果從桌面切換到手機，移除行內樣式，讓 CSS 的 @media 樣式生效
            const allTextElements = document.querySelectorAll(selectors);
            allTextElements.forEach(element => {
                element.style.fontSize = ''; // 清除行內 style
            });
            // 確保按鈕狀態切回預設 (可選，通常手機版不會顯示這些按鈕)
            sizeButtons.forEach(btn => {
                btn.classList.remove('selected', 'unselected');
            });
            document.getElementById(DEFAULT_SIZE_KEY)?.classList.add('selected');
        }
    });

    // --- 2. 按鈕點擊時：切換大小並儲存偏好 (只在桌面版有作用) ---
    sizeButtons.forEach(button => {
        button.addEventListener('click', (event) => {
            // 【新增檢查】點擊事件只在電腦版時執行字體變換和儲存
            if (!desktopMediaQuery.matches) {
                 // 阻止在手機版點擊按鈕產生任何效果 (如果按鈕意外出現在手機版)
                 return;
            }
            ...
            // 執行字體大小更新和按鈕狀態更新
            updateFontSizes(buttonId);

            // 【重要】將新的字體大小儲存到 localStorage
            localStorage.setItem(STORAGE_KEY, buttonId);

            // (可選) 點擊後關閉字體選擇器
            // fontSizeArea?.classList.remove('active');
        });
    });

// --- 字體大小彈出按鈕邏輯 Start (保持不變) ---
// 這裡假設字體大小按鈕在手機版是隱藏的，如果沒有隱藏，
// 點擊事件監聽器仍然會觸發，但字體變換邏輯不會執行。
fontsizeBtn?.addEventListener('click', (e) => {
    e.preventDefault();
    
    // 1. 切換父容器的 'active' class (觸發推擠展開)
    fontSizeArea?.classList.toggle('active');

    // 2. 點擊字體按鈕時，關閉搜尋框 (互相排斥)
    const searchInputContainer = document.getElementById('search-input-container'); 
    const navIcons = document.querySelector('.nav-icons'); 
    
    if (fontSizeArea?.classList.contains('active')) {
...
document.addEventListener('click', (e) => {
    ...
    if (fontSizeArea?.classList.contains('active')) { 
        ...
        // 檢查點擊是否在整個父容器 (.nav-icons-fontsize) 內
        const clickedInsideArea = fontSizeArea.contains(e.target);
...
function updateNavIconsImages(isWhiteBg) {
    const navIcons = document.querySelectorAll('.nav-icons img:not(#toggleDarkMode img), .hamburger-icon');
    const pageShareIcons = document.querySelectorAll('.page-share-fixed .share-icons img, .share-fixed .share-icons img');
    const openInfoIcons = document.querySelectorAll('.openinfo-box-2 img'); 
    // 新增：選擇 mailbox-container 內的 img 元素
    const mailboxIcons = document.querySelectorAll('.mailbox-container img'); 
    
    const isDarkMode = document.documentElement.classList.contains('dark');

    // 擴展：openInfoIcons 和 mailboxIcons 共用此映射表
    const iconColorMap = {
        'img/linklogo_1.svg': 'img/linklogo_1_w.svg',
        'img/linklogo_2.svg': 'img/linklogo_2_w.svg',
        'img/linklogo_3.svg': 'img/linklogo_3_w.svg',
        'img/mailbox/mailbox_01.svg': 'img/mailbox/mailbox_01_w.svg',
        'img/mailbox/mailbox_02.svg': 'img/mailbox/mailbox_02_w.svg',
        'img/mailbox/mailbox_03.svg': 'img/mailbox/mailbox_03_w.svg',
        'img/mailbox/mailbox_04.svg': 'img/mailbox/mailbox_04_w.svg',
    };

    // ------------------------------------------
    // 1. 更新 nav-icons 和 hamburger-icon (邏輯不變)
    // ------------------------------------------
    navIcons.forEach(img => {
        const src = img.getAttribute('src');
        if (!src) {
    ...
    // ------------------------------------------
    // 2. 更新 page-share 和 share-fixed 的圖標 (邏輯不變)
    // ------------------------------------------
    pageShareIcons.forEach(img => {
        const src = img.getAttribute('src');
        if (!src) {
    ...
    // ------------------------------------------
    // 3. 統一處理使用 iconColorMap 的圖標 (openinfo-box-2 和 mailbox-container)
    // ------------------------------------------
    
    // 建立一個包含所有需要映射處理的圖標陣列
    const mappableIcons = [...openInfoIcons, ...mailboxIcons];

    mappableIcons.forEach(img => {
        const src = img.getAttribute('src');
        if (!src) {
            console.warn('可映射圖標 src 為空:', img);
            return;
        }
        
        let newSrc;
        
        // 檢查 src 是否在映射表中 (key 或 value)
        const isMappedKey = iconColorMap.hasOwnProperty(src);
        const isMappedValue = Object.values(iconColorMap).includes(src);

        if (isDarkMode) {
            // 夜間模式：從原始圖標 (key) 切換到白色圖標 (value)
            newSrc = isMappedKey ? iconColorMap[src] : src; 
            
        } else {
            // 日間模式：從白色圖標 (value) 切換回原始圖標 (key)
            if (isMappedValue) {
                // 找出哪個 key 對應到當前的 src (value)
                const originalSrc = Object.keys(iconColorMap).find(key => iconColorMap[key] === src);
                newSrc = originalSrc || src;
            } else {
                newSrc = src;
            }
        }
        
        // 只有在 newSrc 與 src 不同時才更新屬性
        if (newSrc !== src) {
            console.log(`更新映射圖標: ${src} -> ${newSrc}`);
            img.setAttribute('src', newSrc);
        }
    });
}
...
// 【修改點 1】if (teamItems.length > 0 && modalCloseBtns.length > 0 && menuOverlayLeader) 
if (teamItems.length > 0 && modalCloseBtns.length > 0 && menuOverlayLeader) {
    ...
    const closeModal = (currentModal) => {
        ...
        if (activeModals.length === 0) {
            // 【修改點 2】直接操作元素，查詢新的類別選擇器
            // 原本：document.querySelector('.menu-overlay')?.classList.remove('show');
            document.querySelector('.menu-overlay-leader')?.classList.remove('show'); 
        }
    ...
    teamItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // 使用 menuOverlayLeader 變數來新增 'show' 類別 (此處已在您上一個請求中修改)
            menuOverlayLeader?.classList.add('show');
            ...
            if (menuOverlayLeader) { // 增加一個檢查，確保變數存在 (此處已在您上一個請求中修改)
...
const line = document.querySelector('.top-bg .moving-line2');
if (line) {
    function getVisibleLeaderPic() {
        const p1 = document.querySelector('.leader-pic img');
        const p2 = document.querySelector('.leader-pic2 img');

        // Helper：檢查是否顯示
        const isVisible = el => el && el.offsetParent !== null;

        if (isVisible(p1)) return p1;
        if (isVisible(p2)) return p2;

        return null;
    }
    const wrap = document.querySelector('.wrap-page4');
    // 計算並更新 moving-line2 的最大高度，以避免超出 .leader-pic 範圍
    function updateMovingLineHeight() {
        const leaderPic = getVisibleLeaderPic();

        if (!wrap || !leaderPic) return;

        // 取得元素相對 viewport 的座標
        const wrapRect = wrap.getBoundingClientRect();
        const leaderRect = leaderPic.getBoundingClientRect();

        // 計算 leaderPic 底部相對 wrap 的距離
        const leaderOffset = leaderRect.bottom - wrapRect.top;

        // 最大可移動距離 = leader 底部 - movingLine 高度
        const maxTop = Math.max(leaderOffset - line.offsetHeight, 0);

        // 設定 CSS 變數
        const topBg = document.querySelector('.top-bg');
        topBg.style.setProperty('--line-max', `${Math.max(maxTop, 0)}px`);
    }

    // 等整個頁面 (含圖片) 載入後再計算
    function initMovingLine() {
        const leaderPic = getVisibleLeaderPic();
        const img = leaderPic ? leaderPic.querySelector('img') : null;

        if (img && !img.complete) {
            img.addEventListener('load', updateMovingLineHeight);
        } else {
            updateMovingLineHeight();
        }

        // resize 時重新計算
        window.addEventListener('resize', () => {
            requestAnimationFrame(updateMovingLineHeight);
        });
    }

    // 初始呼叫
    window.addEventListener('load', initMovingLine);
}
